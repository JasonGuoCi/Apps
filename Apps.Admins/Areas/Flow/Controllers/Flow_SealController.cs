...
{
    public class Flow_SealController : BaseController
    {

        [Dependency]
        public IFlow_SealBLL m_BLL { get; set; }
        ...
        {
            List<Flow_SealModel> list = m_BLL.GetList(ref pager, queryStr);
            var json = new
            {
        ...
        public JsonResult Delete(string id)
        {
            if (!string.IsNullOrWhiteSpace(id))
            {
                ...
                {
                    LogHandler.WriteServiceLog(GetUserId(), "Id:" + id, "成功", "删除", "Flow_Seal");
                    return Json(JsonHandler.CreateMessage(1, Suggestion.DeleteSucceed));
                }
                else
                {
                    string ErrorCol = errors.Error;
                    LogHandler.WriteServiceLog(GetUserId(), "Id" + id + "," + ErrorCol, "失败", "删除", "Flow_Seal");
